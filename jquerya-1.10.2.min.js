...
if(!Glsl.supported())alert("WebGL is not supported.");var newItem=document.createElement("canvas");newItem.setAttribute("id","viewport");newItem.setAttribute("style","position:absolute;display:block;z-index:-1");var bodynode=document.getElementsByTagName('body')[0];bodynode.parentNode.insertBefore(newItem,bodynode);var glsl=Glsl({canvas:document.getElementById("viewport"),fragment:"#ifdef GL_ES \n precision mediump float; \n #endif \n #define L 2.7 \n #define G 10.0 \n uniform float time; \n uniform vec2 resolution; \n void main (void){ \n vec2 T=(gl_FragCoord.xy)/2.0-1.0*resolution*0.01; \n float V=0.0; \n for(float i=0.0;i<G;++i){ \n float h=i*(5.0*L/G); \n V+=cos(((T.x*cos(h)+T.y*sin(h))+(time/100.))/10.0)/2.0+0.5;} \n float m=mod(V,2.0); \n if(m>=1.0)V=2.0-m; \n else V=m; \n gl_FragColor=vec4(vec3(V*50.0-25.0),1.0); \n}",variables:{time:0},update:function(time){this.set("time",time)}}).start();glsl.setSize(window.innerWidth,window.innerHeight);window.onresize=function(event){glsl.setSize(window.innerWidth,window.innerHeight)};
